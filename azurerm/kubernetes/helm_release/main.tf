...
provider "helm" {
  kubernetes {
    ...
    host                   = data.terraform_remote_state.aks.outputs.endpoint
    cluster_ca_certificate = base64decode(data.terraform_remote_state.aks.outputs.certificate_authority.0.data)
    exec {
      api_version = "client.authentication.k8s.io/v1beta1"
      args        = ["aks", "get-credentials", "--name", data.azurerm_kubernetes_cluster.cluster.name, "-g", data.terraform_remote_state.aks.outputs.resource_group_name]
      command     = "az"
